...
func _process(delta: float) -> void:
    ...
    var thirst_drain := THIRST_DRAIN_WORKING if _is_working else THIRST_DRAIN_NORMAL
...
func _draw() -> void:
    ...
    var tr := thirst / MAX_THIRST
    if tr > 0.0:
        var tc := Color(0.2, 0.6, 0.9) if tr > 0.5 else (Color(0.9, 0.8, 0.2) if tr > 0.25 else Color(0.9, 0.3, 0.1))
        draw_rect(Rect2(bx, THIRST_BAR_Y, BAR_WIDTH * tr, BAR_HEIGHT), tc)
