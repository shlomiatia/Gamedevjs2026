...
func add_resource(scene: PackedScene) -> void:
    ...
    resource.position = Vector2(0, -get_child_count() * resource.SPACE)
    add_child(resource)
    _notify_new_resource()

func add_existing_resource(node: Node2D) -> void:
    node.position = Vector2(0, -get_child_count() * node.SPACE)
    add_child(node)
    _notify_new_resource()

func _notify_new_resource() -> void:
    if _coordination_manager != null and _resource_type >= 0:
        _coordination_manager.notify_free_resource(_resource_type, self)
...
func collect(collector: Node) -> Node2D:
    ...
    assert(idx >= 0, "collect called by unreserved collector")
    _reserved_by.remove_at(idx)
    assert(get_child_count() > 0, "collect called on empty pile")
