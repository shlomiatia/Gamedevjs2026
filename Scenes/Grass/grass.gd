...
func find_wheat_planting_tile(near_pos: Vector2, occupied_tiles: Dictionary, extra_occupied: Dictionary = {}) -> Vector2i:
	var start := _grass_layer.local_to_map(near_pos)
	var bounds := _grass_layer.get_used_rect()
	var queue: Array[Vector2i] = [start]
	var visited: Dictionary = {start: true}
	while not queue.is_empty():
		var tile: Vector2i = queue.pop_front()
		if not occupied_tiles.has(tile) and not extra_occupied.has(tile) and not _wheat_sprites.has(tile) and _grass_layer.get_cell_atlas_coords(tile) == Vector2i(0, 0):
			return tile
		for offset: Vector2i in [Vector2i(1, 0), Vector2i(-1, 0), Vector2i(0, 1), Vector2i(0, -1)]:
			var neighbor := tile + offset
			if not visited.has(neighbor) and bounds.has_point(neighbor):
...
func remove_wheat_tile(tile: Vector2i) -> void:
