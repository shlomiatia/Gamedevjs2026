...
func get_pile_for_type(type: int) -> ResourcePile:
	return _output_pile if type == CoordinationManager.ResourceType.PLANK else null
...
func on_placed(spawn_parent: Node2D, map: Map, coordination_manager: Node, _forest: Forest) -> void:
	...
	_output_pile.setup(coordination_manager, CoordinationManager.ResourceType.PLANK)
...
func complete_construction() -> void:
	...
	miller.setup(self, _map, _coordination_manager, CoordinationManager.ResourceType.LOG, PlankScene, _output_pile, WORKER_NAME)
