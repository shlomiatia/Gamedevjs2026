...
const BeerScene = preload("res://Scenes/Resources/Beer/Beer.tscn")
...
func get_pile_for_type(type: int) -> ResourcePile:
	return _output_pile if type == CoordinationManager.ResourceType.BEER else null
...
func on_placed(spawn_parent: Node2D, map: Map, coordination_manager: Node, _forest: Forest) -> void:
	...
	_output_pile.setup(coordination_manager, CoordinationManager.ResourceType.BEER)
...
func complete_construction() -> void:
	...
	worker.setup(self, _map, _coordination_manager, BeerScene,
		CoordinationManager.ResourceType.WHEAT, CoordinationManager.ResourceType.LOG, _output_pile, WORKER_NAME)
