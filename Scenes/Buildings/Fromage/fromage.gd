...
const CheeseScene = preload("res://Scenes/Resources/Cheese/Cheese.tscn")
...
func get_pile_for_type(type: int) -> ResourcePile:
	return _output_pile if type == CoordinationManager.ResourceType.CHEESE else null
...
func on_placed(spawn_parent: Node2D, map: Map, coordination_manager: Node, _forest: Forest) -> void:
	...
	_output_pile.setup(coordination_manager, CoordinationManager.ResourceType.CHEESE)
...
func complete_construction() -> void:
	...
	worker.setup(self, _map, _coordination_manager, CheeseScene,
		CoordinationManager.ResourceType.MILK, CoordinationManager.ResourceType.LOG, _output_pile)
