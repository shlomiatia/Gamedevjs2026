class_name Gritsmill
extends Node2D

const SIZE_X := 5
const SIZE_Y := 1
const BUILDING_NAME := "Flour mill"
const WORKER_NAME := "Flour miller"

const MillerScene = preload("res://Scenes/Workers/Miller/Miller.tscn")
const FlourScene = preload("res://Scenes/Resources/Flour/Flour.tscn")
...
func get_pile_for_type(type: int) -> ResourcePile:
	return _output_pile if type == CoordinationManager.ResourceType.FLOUR else null
...
func on_placed(spawn_parent: Node2D, map: Map, coordination_manager: Node, _forest: Forest) -> void:
	...
	_output_pile.setup(coordination_manager, CoordinationManager.ResourceType.FLOUR)
...
func complete_construction() -> void:
	...
	miller.setup(self, _map, _coordination_manager, CoordinationManager.ResourceType.WHEAT, FlourScene, _output_pile, WORKER_NAME)
