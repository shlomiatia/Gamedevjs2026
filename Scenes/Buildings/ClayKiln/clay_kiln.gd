...
func get_pile_for_type(type: int) -> ResourcePile:
	return $Building.get_output_pile() if type == CoordinationManager.ResourceType.BRICK else null
...
func on_placed(spawn_parent: Node2D, map: Map, coordination_manager: Node, _forest: Forest) -> void:
	...
	$Building.get_output_pile().setup(coordination_manager, CoordinationManager.ResourceType.BRICK)
...
func complete_construction() -> void:
	...
	worker.setup(self, _map, _coordination_manager, BrickScene, CoordinationManager.ResourceType.CLAY, CoordinationManager.ResourceType.LOG)
